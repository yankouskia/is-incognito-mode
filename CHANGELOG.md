...
- 40edb80: Add opt-in verdict caching: `detectIncognito()` / `isIncognito()` accept `cache?: boolean`.
...
- 153b48e: Add bounded, cancelable detection: `detectIncognito()` / `isIncognito()` now accept `timeoutMs` and an `AbortSignal` `signal`.
